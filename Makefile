...
DOCKER_DIR_GENERATE := $(if $(BIND_DIR),-v "/root/deeplearning/$(BIND_DIR)")
DOCKER_MOUNT := $(if $(BIND_DIR),-v "$(CURDIR)/$(BIND_DIR):/root/deeplearning/$(BIND_DIR)")

DOCKER_MOUNT := $(if $(DOCKER_MOUNT),$(DOCKER_MOUNT),-v "/root/deeplearning/$(BIND_DIR)")
...
UBUNTU_IMAGE := deeplearning4j-dev:01
